...
# Introduction
This is a mini application implementing REST CRUD operations using Spring Boot with Spring Reactive Web.
The data layer is wrapped around the reactor API, I am not an expert, so I don't really know if this is the right way
of wrapping a blocking API into a Rx API, but at least I tried and it works. Hope it helps for learning.
You may try the Spring r2dbc extensions but those libraries are still in Beta, and it is most likely your app won't even
start even if your code is ok, so for now, just forget about native support of Rx for JDBC, it just does not exist.
...
- [Asp.Net Web Api 2 + NHibernate](https://github.com/melardev/.NetWebApiADO.NetCrud)
...
- [Asp.Net Web Api 2 + NHibernate](https://github.com/melardev/.NetWebApiADO.NetCrudPagination)
